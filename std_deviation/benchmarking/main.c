...
#define FILE_NAME "dataset_50M.txt" //<-- I'll give you seg fault if I dont exist !
#define GPU "GeForce"

#define ITERATIONS 6
double sequential_time[ITERATIONS];
double parallel_time[ITERATIONS];
int current_iter;
...
int run ()
{
 ...
 //START OPENCL CALCULATIONs
  ...
  sequential_time[current_iter] = seq_time;
  printf("PARALLEL TIME: %lf\n", mean1_time+diff_time+sum_time);
  parallel_time[current_iter] = mean1_time+diff_time+sum_time;
  current_iter++;
...
int main(void) {
  ...
  double avg_pl_time = 0;
  for (int i = 1; i < ITERATIONS; i++) {
    printf("sequential time: %lf    parallel time: %lf\n", sequential_time[i], parallel_time[i]);
    avg_seq_time += sequential_time[i];
    avg_pl_time += parallel_time[i];
  }

  printf("\n AVERAGE SEQ TIME: %lf      AVG PARALLEL TIME: %lf\n", avg_seq_time/(ITERATIONS-1), avg_pl_time/(ITERATIONS-1));
}